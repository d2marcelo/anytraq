package com.brtracker.shared.utils.cache;
...
public class CacheClient {
	...
	private CacheClient() {
		try {
			...
			memCachedServerAddress = memcached.getUrl();
			client = new MemcachedClient[5];
			for (int i = 0; i <= 4; i++) {
				MemcachedClient c = new MemcachedClient(
						 new InetSocketAddress(memCachedServerAddress, Integer.parseInt(memcached.getPort())));
				client[i] = c;
			}
		} catch (Exception e) {
			e.printStackTrace();
