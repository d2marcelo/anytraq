...
import com.jcraft.jsch.UserInfo;
	...
	public  boolean runCmd (String cmd){
		  try{
	      logger.logDebug("Running Command: "+ cmd);
	      JSch jsch=new JSch();
	        ...
	        while(in.available()>0){
	          ...
	          if(i<0)break;
	            String out = new String(tmp, 0, i);
	          	 logger.logDebug("OUTPUT: "+out);
	          	setSshOutput(out);

	        }
	        if(channel.isClosed()){
	        	if (channel.getExitStatus() == 0){
	        		 logger.logDebug("Command completed successfuly.");
	        		setExitStatus(0);
	        	}
	        	else {
	...
	public boolean run (String cmd) {
		...
		if (runCmd(cmd)) {
			return true;
			 ...
			 logger.logInfo("command  "+cmd+" executed on server "+host +" returned exit status "+getExitStatus());
			 logger.logInfo("command returned status "+getExitStatus());
			return false;
		}
